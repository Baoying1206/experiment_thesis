...
Zhao et al.~\citeyearpar{zhao2026llms} provide the most directly relevant motivation for RQ1: they show that LLMs encode \textit{harmfulness} and \textit{refusal} in separable directions in activation space. This finding suggests that the jailbreak direction need not simply be the negation of the refusal direction, motivating our investigation of their geometric relationship across languages.

\subsection{Vector-based Defense and Attack}

Wang et al.~\citeyearpar{wang2025surgical} show that ablating the refusal direction via single-vector intervention reduces false refusals on benign inputs, demonstrating that refusal direction surgery can be applied with surgical precision. Piras et al.~\citeyearpar{piras2026som} extend this to multi-directional suppression, showing that using multiple refusal directions improves the robustness of vector-based attacks. These works establish the effectiveness and versatility of activation-steering for safety control. Our defense experiments addressing RQ3 adopt the same intervention mechanism but apply it in the cross-lingual setting.
