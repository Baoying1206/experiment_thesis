...
For HRL languages, jailbreaking should correspond to suppressing the refusal direction, yielding $\cos(\hat{\mathbf{j}}_l, \hat{\mathbf{r}}_l) \approx -1$. For LRL languages, where no refusal direction has been established during training, the jailbreak vector is expected to be geometrically unrelated to $\mathbf{r}_l$, predicting $\cos(\hat{\mathbf{j}}_l, \hat{\mathbf{r}}_l) \approx 0$. High values of $S_{ij}$ across language pairs would indicate a shared geometric structure of jailbreak behavior, providing motivation for the transfer experiments in RQ2 and RQ3.
...
\paragraph{Expected Observations.}
If jailbreak vectors share a cross-lingual geometric structure, HRL-to-HRL transfer should increase bypass rates above baseline. If LRL vectors can also transfer despite being orthogonal to $\mathbf{r}_l$, this suggests that attack efficacy does not depend on the suppression of an existing refusal direction. A null result in LRL-to-HRL transfer would support the view that only aligned languages share a common jailbreak mechanism.
...
\paragraph{Expected Observations.}
Self-defense should reduce compliance rates in HRL languages, consistent with prior findings~\citep{arditi2024refusal}. For LRL languages, where $\mathbf{j}_l$ cannot be extracted due to near-100\% bypass rates, only cross-lingual defense is applicable. If subtracting an HRL jailbreak vector reduces compliance in an LRL target, this constitutes inference-time alignment injection: a training-free method to introduce refusal behavior into a language that was never safety-tuned. A null result would indicate that cross-lingual defense does not generalise to settings where the target language lacks alignment.
