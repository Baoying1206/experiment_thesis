...
For each language $l$, we partition the baseline harmful test completions into two groups based on WildGuard~\citep{wildguard} evaluation:
%
...
We use an extended version of the PolyRefuse dataset~\citep{polyrefuse}, which we term \textbf{PolyRefuse Enhanced}, covering 16 languages including two new low-resource additions: Swahili and Amharic, translated using Google Translate. The dataset comprises the following splits per language:
